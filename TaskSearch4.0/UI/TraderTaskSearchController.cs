...
namespace TaskSearch.UI
{
    internal sealed class TraderTaskSearchController : MonoBehaviour
    {
        ...
        internal static TraderTaskSearchController Attach(QuestsListView view, TraderClass trader)
        {
        ...
        internal void OnVisibilityUpdated()
        {
            ...
            List<QuestClass> candidates = new List<QuestClass>(rows.Length);

            for (int i = 0; i < rows.Length; i++)
            {
                QuestClass quest = rows[i] != null ? rows[i].Quest : null;

...
namespace TaskSearch.Search
{
    internal sealed class TaskSearchIndex
    {
        ...
        internal void Rebuild(IEnumerable<QuestClass> quests)
        {
            if (quests == null)
            {
                _entries.Clear();
            ...
            List<QuestClass> displayable = new List<QuestClass>();
            int total = 0;
            int signature = 17;
            ...
            foreach (QuestClass quest in displayable)
            {
                if (!TryIndex(quest))
                {
                    failed++;
        ...
        private static bool IsDisplayable(QuestClass quest)
        {
            try
            {
        ...
        private static int SafeStatusCode(QuestClass quest)
        {
            try
            {
        ...
        internal int IndexMissing(IEnumerable<QuestClass> quests)
        {
            if (quests == null)
            {
                return 0;
        ...
        private bool TryIndex(QuestClass quest)
        {
            string id = SafeId(quest);
        ...
        private void CacheQuestName(QuestClass quest)
        {
            try
            {
                string id = quest.Id;
        ...
        internal TaskSearchEntry GetEntry(QuestClass quest)
        {
            string id = SafeId(quest);
        ...
        internal List<TaskSearchResult> Search(IReadOnlyList<QuestClass> candidates, TaskSearchQuery query)
        {
        ...
        private static string SafeId(QuestClass quest)
        {
            if (quest == null)
            {
                return null;
